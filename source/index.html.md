...
| `deeplink`                      | Optionally, start on a specific step. Accepts an object. <table><tr><th>Property</th><th>Value</th></tr><tr><td>`step`<h6>required</h6></td><td>Acceptable values: `search-company` or `login` (if `login`, then `connectorId` and `companyName` are required)</td></tr><tr><td>`connectorId`</td><td>Required if the step is `login`. Accepts the [ID](#connector-search) of the connector</td></tr><tr><td>`companyName`</td><td>Required if the step is `login`. Accepts a string of the company name.</td></tr></table> |
